...
def get_model(prm):
    ...
    elif model_name == 'DAFcNet3':
        model = DAFcNet3(model_type, model_name, linear_layer, conv2d_layer, task_info)
    ...
    model.to(prm.device) # always use GPU
...
# -------------------------------------------------------------------------------------------
#  FcNet3- domain adaptation
# -------------------------------------------------------------------------------- -----------
class DAFcNet3(general_model):
    def __init__(self, model_type, model_name, linear_layer, conv2d_layer, task_info):
        super(DAFcNet3, self).__init__()
        self.model_type = model_type
        self.model_name = model_name
        self.layers_names = ('FC1', 'FC2', 'FC3', 'FC_out')
        input_shape = task_info['input_shape']
        output_shape = task_info['output_dim']
        n_classes = task_info['n_classes']
        input_size = input_shape[0] * input_shape[1] * input_shape[2]
        ...
        self.fc2 = linear_layer(n_hidden1, n_hidden2)
        self.fc3 = linear_layer(n_hidden2, n_hidden3)
        self.fc_out = linear_layer(n_hidden3, output_shape)

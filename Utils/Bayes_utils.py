...
def run_test_Bayes(model, source_test_loader, target_test_loader, loss_criterion, prm, verbose=1):

    if len(source_test_loader) or len(target_test_loader) == 0:
        return 0.0, 0.0

    if prm.test_type == 'MaxPosterior':
        info = run_test_max_posterior(model, source_test_loader, target_test_loader, loss_criterion, prm)
    elif prm.test_type == 'MajorityVote':
        info = run_test_majority_vote(model, source_test_loader, target_test_loader, loss_criterion, prm, n_votes=5)
    elif prm.test_type == 'AvgVote':
        info = run_test_avg_vote(model, source_test_loader, target_test_loader, loss_criterion, prm, n_votes=5)
    else:
        raise ValueError('Invalid test_type')
    print('Source Test Accuracy: {:.3} ({}/{}), Target Test Accuracy: {:.3} ({}/{}), Test loss: {:.4}'.format(
        float(info['source_test_acc']), info['n_source_correct'], info['n_source_samples'],
        float(info['target_test_acc']), info['n_target_correct'], info['n_target_samples'],
        float(info['test_loss'])))
    if verbose:
        pass
    return info['test_acc'], info['test_loss']


def run_test_max_posterior(model, source_test_loader, target_test_loader, loss_criterion, prm):

    n_source_test_samples = len(source_test_loader.dataset)
    n_target_test_samples = len(target_test_loader.dataset)

    model.eval()
    test_loss = 0
    n_source_correct = 0
    n_target_correct = 0
    for source_batch_data, target_batch_data  in zip(source_test_loader, target_test_loader):
        ...
        test_loss += loss_criterion( source_y, source_outputs, target_outputs)  # sum the mean loss in batch
        n_source_correct += count_correct(source_outputs, source_y)
        n_target_correct += count_correct(target_outputs, target_y)

    test_loss = l2_reg(model)/(prm.b*prm.c)
    test_loss /= np.max(n_source_test_samples, n_target_test_samples)
    source_test_acc = n_source_correct / n_source_test_samples
    target_test_acc = n_target_correct / n_target_test_samples
    info = {'source_test_acc': source_test_acc, 'n_source_correct': n_source_correct,
            'n_source_samples': n_source_test_samples, 'target_test_acc': target_test_acc,
            'n_target_correct': n_target_correct, 'n_target_samples': n_target_test_samples,
            'test_type': 'max_posterior',
            'n_test_samples': np.max(n_source_test_samples, n_target_test_samples), 'test_loss': get_value(test_loss)}
    return info


def run_test_majority_vote(model, source_test_loader, target_test_loader, loss_criterion, prm, n_votes=9):
    """hard majority vote"""
...
def run_test_avg_vote(model, source_test_loader, target_test_loader, loss_criterion, prm, n_votes=5):
    """soft majority vote"""
    n_test_samples = len(test_loader.dataset)
    n_test_batches = len(test_loader)
    model.eval()
